...
- Tests: `tests/test_http_review_lows.py`. Each test fails when its fix is reverted.
- Not changed (an owner decision): repository branch names still appear in discovery
  freshness, as they already do in `/v1/status` (review finding 7).
...
- `src/beacon/main.py`: `serve` gains `--transport` (`stdio` | `http`), `--host`
  ...
  `_run_mcp_server(transport, host, port)` runs the same `mcp` object; http goes through
  `_run_mcp_http`, which pre-binds the socket (`http_bind_failed`, exit 2), prints
  `[beacon] MCP http listening on http://<host>:<port>/mcp (pid=...)` to stderr, and runs
  `mcp.http_app(path="/mcp")` under uvicorn with no access log. `_serve_with_env` threads
  the three keyword-only parameters through with stdio defaults.
- `src/beacon/loopback_guard.py` (new): ASGI guard that refuses a non-loopback `Host`
  (421) or browser `Origin` (403), so a web page cannot reach the server by DNS
  rebinding. Local because fastmcp's own guard is not in the 3.2.4 floor.
...
- Tests: `tests/test_mcp_http.py` (new) — CLI refusal cases, bind failure, guard unit
  cases, plus a black-box parity test that starts both a stdio and an HTTP server on one
  snapshot, asserts identical tool lists and identical non-error payloads for all seven
  tools, and checks forged Host/Origin requests get 421/403. Existing tests untouched.
